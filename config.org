...
emacs settings (currently this file), EA related projects, blog writing,
job-finding, sonar project, thesis, shopping, phone setup, existence chores, a refile bucket
alternatively, doom has a thing where it's possible to file captures =SPC-X= to project specific files, all centralized in the [[~/org]] folder.
Unfortunately, I'm not sure projectile projects really cleave what I'm up to at the joints; for example, the sonar project frequently involves fixing things in the scattering transform.
These two are not mutually exclusive.
Need to determine if the current state of e.g. this file is integrable.
Also, the current location [[~/org]] isn't a good place for that, because it's already full of stuff for org.
...
#+RESULTS:
| W | Weekly review        | agenda                 |   | ((org-agenda-start-day -14d) (org-agenda-span 14) (org-agenda-start-on-weekday 1) (org-agenda-start-with-log-mode '(closed)) (org-agenda-skip-function '(org-agenda-skip-entry-if 'notregexp ^\*\* DONE ))) |
...
#+end_src
update the feed whenever emacs is idle for 10 minutes
#+begin_src elisp :tangle no
