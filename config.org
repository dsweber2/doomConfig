...
turns out I just forgot to set this
#+begin_src elisp :tangle yes
...
** Projectile
custom-suffixes is telling projectile to ignore files with this ending. Atm it's to avoid pdfs, images, and R documentation files (since those are super redundant with Roxygen).
Caching has problems with git repositories, where any branch changes make it impossible to find the file. Also the search is super fast anyways, so turning this off by default. If a project is slow, add it to the =.dir-envs.el= in the project
#+begin_src emacs-lisp :tangle yes
(after! projectile
  :config
...
This is also causing an emacsql column mismatch error ("table pullreq has 39 columns but 38 values were supplied")
...
* SQL stuff
** Connection presets
Use =M-x sql-connect= to pick a named connection. Add entries to =sql-connection-alist= as needed.
Passwords are intentionally omitted here — use =~/.pgpass= (format: =hostname:port:database:username:password=) so postgres skips the prompt automatically.
#+begin_src elisp :tangle yes
(after! sql
...
From [[https://github.com/hadronzoo/theme-changer][here]]. For actual location data, see localConfig.el
